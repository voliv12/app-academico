...
Class Publicaciones extends CI_controller{
    ...
    {              
        ...
        {             
            ...
            $datos['cap_int'] = 0;
            $datos['p_invest'] = 0;   
            $action['action'] = "informe/publicaciones/reporte_general";
    ...
    function ponencias()
    {
        ...
        { 
            ...
            if(($fecha_de != NULL) && ($fecha_hasta != NULL))
            {                                                       
                $this->load->model('publicaciones_model');                
                $ponencias['ponencias'] = $this->publicaciones_model->listar_ponencias($fecha_de, $fecha_hasta);
            }else{
            ...
            $ponencias['periodo'] = implode("/", array_reverse( preg_split("/\D/", $fecha_de) ) )." - ".implode("/", array_reverse( preg_split("/\D/", $fecha_hasta) ) );
            //print_r($ponencias);                                                                       
            $datos_plantilla['titulo'] = "Informe"; 
            $datos_plantilla['contenido'] = $this->load->view('informe/ponencias_view',$ponencias,TRUE);
            $this->load->view('informe_view', $datos_plantilla);
        }
    }                
}
